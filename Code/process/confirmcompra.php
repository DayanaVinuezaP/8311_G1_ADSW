...
if($num>0){
  ...
  $nitC=$data['Codigo_Cliente'];
  $StatusV="Pendiente";
  
  /*Insertando datos en tabla venta*/
  consultasSQL::InsertSQL("venta", "Fecha, Codigo_Cliente, Descuento, TotalPagar, Estado", "'".date('d-m-Y')."','".$nitC."','0','".$_SESSION['sumaTotal']."','".$StatusV."'");
  
  /*recuperando el número del pedido actual*/
  $verId=ejecutarSQL::consultar("select * from venta where Codigo_Cliente='$nitC' order by NumPedido desc limit 1");
  ...
  for($i = 0;$i< $_SESSION['contador'];$i++){
      consultasSQL::InsertSQL("detalleVenta", "NumPedido, CodigoProd, CantidadProductos", "'$Numpedido', '".$_SESSION['producto'][$i]."', '1'");
    ...
    echo '<img src="imagenes/img/ok.png" class="center-all-contens"><br>El pedido se ha realizado con éxito';
  }else{
    echo '<img src="imagenes/img/error.png" class="center-all-contens"><br>No has seleccionado ningún producto, revisa el carrito de compras';
  }

}else{
    echo '<img src="imagenes/img/error.png" class="center-all-contens"><br>El nombre o contraseña invalidos';
}
